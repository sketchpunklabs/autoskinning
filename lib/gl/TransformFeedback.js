...
export default class TransformFeedback{
    ...
    static compute( renderer, sh, bufMap, exeCnt=0 ){        
        ...
        // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        this.unbindInputs( gl, sh );
        this.unbindOutputs( gl, sh );
        this.postCompute( gl );
    }
    ...
    static unbindInputs( gl, sh ){
        // Unbind  gl buffer
        gl.bindBuffer( gl.ARRAY_BUFFER, null );
        
        for( const [k,v] of Object.entries( sh.inputs ) ){    
            // Needed else the buf doesn't really get unbound
            // And you can get errors about TF buffer is bound to 
            // a non-transform-feedback target.

            // Known to fix the problem when doing ping-pong execution
            // then using output as input in another compute before pong
            switch( v.type ){
                case 'float': case 'vec2': case 'vec3': case 'vec4':
                    gl.vertexAttribPointer( v.loc, 1, gl.FLOAT, false, 0, 0 ); 
                    break;

                case 'uint': case 'uvec2': case 'uvec3': case 'uvec4' :
                    gl.vertexAttribIPointer( v.loc, 1, gl.UNSIGNED_INT, false, 0, 0 ); break;
                    break;

                default: console.log( 'TransformFeedback.unbindInputs - Unknown type : ', v.type ); break;
            }

            // Turn off Attribute slot
            gl.disableVertexAttribArray( v.loc );
        }
    }
    ...
    static unbindOutputs( gl, sh ){
        for( const [k,v] of Object.entries( sh.outputs ) ){
            gl.bindBufferBase( gl.TRANSFORM_FEEDBACK_BUFFER, v.loc, null );       
        }
    }
