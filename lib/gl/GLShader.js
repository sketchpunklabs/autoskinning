...
export default class GLShader{
    ...
    static forGPGPU( fSrc, vSrc=null ){
        // Compile
        const o = build( vSrc || GPGPU_VERT, fSrc );
        if( !o ) return null;
...
function parseUniforms( prog, src, out ){
    ...
    const reUniform = /uniform\s+(highp|mediump|lowp)?\s*(\w+)\s+(\w+)\s*;/g; // out vec3 color;
    const gl        = GLContext.ctx;
    let result;
    let loc;
    // console.log( src );
    ...
    while( (result = reUniform.exec(src)) ){
        loc = gl.getUniformLocation( prog, result[3] );
        // console.log( result );
        if( loc !== null )  out[ result[3] ] = { type: result[2], loc: loc, value: null };
        else                console.log( 'Uniform missing, may not be used : ', result[3] );
    }
}
// #endregion
