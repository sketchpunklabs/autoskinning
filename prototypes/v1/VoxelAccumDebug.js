...
export default function VoxelCrawlDebug( vox ){
    const iCnt = 4; // Component Count
    const buf  = new Float32Array( vox.count * iCnt );//.fill( -1.0 );
    const buf2 = new Float32Array( vox.count );//.fill( -1.0 );
    ...
    iGeo.setAttribute( 'inst', new THREE.InstancedBufferAttribute( buf2, 1 ) );
    iGeo.setAttribute( 'skinWeight', new THREE.InstancedBufferAttribute( buf, iCnt ) );
    iGeo.setAttribute( 'skinIndex', new THREE.InstancedBufferAttribute( buf, iCnt ) );
    ...
    const mat = new THREE.RawShaderMaterial({
        ...
        void main(){
            ...
            if( inst <= -5.0 ){
                ...
                // fragColor = ( wgt >= 0.0 )? vec3( 1.0, 0.0, 0.0 ) : vec3( 0.4 );
                
                fragColor = mix(
                    vec3( 1.0, 0.0, 0.0 ),
                    vec3( 0.0, 1.0, 1.0 ),
                    pow( wgt / vMaxDist, 0.7 )
                );

                if( wgt == 0.0 ) fragColor = vec3( 0.0, 1.0, 0.0 );
